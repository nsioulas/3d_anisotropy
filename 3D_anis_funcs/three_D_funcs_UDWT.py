...
def local_structure_function(dB, B_l , V, tau, return_unit_vecs=False, five_points_sfunc =True):
    '''
    Parameters:
...
def structure_functions_3D(indices, qorder,mat):
    """
    Parameters:
...
def estimate_pdfs_3D(indices, mat):
    """
...
def estimate_3D_sfuncs(B,V, dt, Vsw, di, conditions, qorder, tau_values, estimate_PDFS=False, return_unit_vecs=False, five_points_sfuncs= True, return_coefs=False):
    """
    ...
    for jj, tau_value in enumerate(tau_values):
        ...
        dB, VBangle, Phiangle, dB_l_perp_hat, B_l_hat, B_perp_2_hat,V_l_hat = local_structure_function(B,V, int(tau_value), return_unit_vecs, five_points_sfunc=five_points_sfuncs)
    ...
    return flucts, l_di, sf_ell_perp.T, sf_Ell_perp.T,  sf_ell_par.T, sf_overall.T, {'All':PDF_all,'ell_par':PDF_ell_par ,'ell_perp':PDF_Ell_perp,'ell_perp':PDF_ell_perp}